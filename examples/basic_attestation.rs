...
fn main() {
    ...
    eprintln!(
        "  UID:        {}",
        hex::encode(attestation.uid.0.to_array())
    );
    eprintln!(
        "  Schema UID: {}",
        hex::encode(attestation.schema_uid.0.to_array())
    );
    ...
    let network_id = env.crypto().sha256(&soroban_sdk::Bytes::from_slice(
        &env,
    ...
    let secret_seed_hex =
        std::env::var("SECRET_SEED").expect("SECRET_SEED is required for --submit");
    ...
    if attester != expected_attester {
        eprintln!(
            "Error: ATTESTER ({attester}) does not match SECRET_SEED account ({expected_attester})"
        );
        std::process::exit(1);
    }
